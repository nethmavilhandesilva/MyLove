...
const App = () => {
  ...
  const startSync = () => {
    // 1. Start audio immediately to unlock browser restriction
    if (audioRef.current) {
    ...
    syncInterval.current = setInterval(() => {
      setSyncProgress(prev => {
        if (prev >= 100) {
          ...
          handlePortalTransition(); // THIS TRIGGERS THE PAGE CHANGE
          return 100;
        }
        return prev + 1;
      });
  ...
  const handlePortalTransition = () => {
    ...
    // Ensure music is definitely playing
    if (audioRef.current) {
    ...
    // AFTER 2 SECONDS IN PORTAL, MOVE TO CINEMATIC UNIVERSE
    setTimeout(() => {
      setPhase('cinematic');
    }, 2500);
  };
          ...
          <motion.div key="cinema" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={styles.universe}>
            ...
            <div style={styles.gallery}>
              {MEMORY_DATA.map((item, i) => (
                <ParallaxPhoto key={i} data={item} index={i} />
              ))}
            </div>
        ...
        {phase === 'letter' && (
          <motion.div key="letter" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={styles.screen}>
             ...
             <div style={styles.letterBox}>
                <AutoTypingLetter text={LETTER_TEXT} />
             </div>
          </motion.div>
        )}
      </AnimatePresence>
...
// --- SUB-COMPONENTS ---
const ParallaxPhoto = ({ data, index }) => (
  <motion.div initial={{ opacity: 0, y: 150 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false, amount: 0.2 }} transition={{ duration: 1.2 }} style={styles.photoNode}>
    <div style={{...styles.parallaxContainer, flexDirection: index % 2 === 0 ? 'row' : 'row-reverse'}}>
      <motion.div whileHover={{ scale: 1.05, rotateY: index % 2 === 0 ? 10 : -10 }} style={styles.imageCard}>
        <img src={data.url} alt="Memory" style={styles.image} />
        <div style={styles.imgOverlay} />
      </motion.div>
      <div style={{...styles.photoInfo, textAlign: index % 2 === 0 ? 'left' : 'right'}}>
        <span style={styles.photoIndex}>0{index + 1}</span>
        <h2 style={styles.photoTitle}>{data.title}</h2>
        <p style={styles.photoWords}>"{data.words}"</p>
      </div>
    </div>
  </motion.div>
);

const AutoTypingLetter = ({ text }) => {
  ...
  return (
    <div style={styles.typewriterContainer}>
      ...
      <p style={styles.typewriterText}>{displayedText}<span>|</span></p>
      {displayedText.length === text.length && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ marginTop: '50px' }}>
          <Heart size={50} fill="#ff0055" color="#ff0055" />
          <p style={{ fontSize: '14px', marginTop: '15px', letterSpacing: '6px' }}>FOREVER YOURS</p>
        </motion.div>
      )}
    </div>
  );
...
const StarField = () => (
  <div style={styles.starLayer}>
    {[...Array(80)].map((_, i) => <div key={i} style={{ position: 'absolute', top: `${Math.random() * 100}%`, left: `${Math.random() * 100}%`, width: '2px', height: '2px', background: '#fff', borderRadius: '50%', opacity: Math.random() }} />)}
  </div>
);
...
const styles = {
  ...
  topBar: { position: 'fixed', top: '30px', left: '50%', transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '12px', letterSpacing: '4px', color: '#ff0055', zIndex: 100, fontWeight: 'bold' },
  authBox: { maxWidth: '400px' },
  glitchTitle: { fontSize: '16px', letterSpacing: '10px', color: '#ff0055', margin: '25px 0' },
  authSub: { fontSize: '12px', color: '#666', lineHeight: '1.8', marginBottom: '45px' },
  ...
  heroScrollHint: { fontSize: '11px', letterSpacing: '6px', color: '#ff0055', marginTop: '30px' },
  gallery: { padding: '0 8%' },
  photoNode: { height: '110vh', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  parallaxContainer: { display: 'flex', gap: '80px', alignItems: 'center', width: '100%', maxWidth: '1100px' },
  ...
  letterTrigger: { height: '60vh', display: 'flex', justifyContent: 'center', alignItems: 'center' },
  finalButton: { background: 'none', border: '2px solid #ff0055', color: '#ff0055', padding: '22px 50px', borderRadius: '60px', cursor: 'pointer', fontSize: '13px', letterSpacing: '5px' },
  letterBox: { maxWidth: '800px', background: 'rgba(255,255,255,0.03)', padding: '70px', borderRadius: '50px', border: '1px solid rgba(255,255,255,0.08)', backdropFilter: 'blur(30px)' },
  typewriterText: { fontSize: '26px', lineHeight: '1.9', color: '#f0f0f0', fontStyle: 'italic' }
};
